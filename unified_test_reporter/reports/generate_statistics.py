...
def main():
    ...
    logger.info('Initializing jenkins Build configuration...')
    running_build = Build(args.job_name, args.build_number)

    if args.job_name:
        logger.info('Inspecting {0} build of {1} Jenkins job for TestPlan '
                    'details...'.format(args.build_number, args.job_name))
        test_plan_name = running_build.generate_test_plan_name()
